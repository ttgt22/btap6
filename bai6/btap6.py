...
def init_centroids(pixels, k):
    # Chọn ngẫu nhiên `k` tâm cụm từ dữ liệu
...
def assign_clusters(pixels, centroids):
    ...
    for pixel in pixels:
        ...
        distances = [euclidean(pixel, centroid) for centroid in centroids]
...
def update_centroids(pixels, clusters, k):
    ...
    new_centroids = []
    for i in range(k):
        # Lấy tất cả các điểm t huộc về cụm `i`
...
# Đọc ảnh từ file
image = Image.open("D:/gkgk/ghgh.jpg")
